...
codeGenECall (Prim name _) exps
    ...
    | otherwise = case name of
                    ...
                    Sqrt  -> mapM k2llvmExp exps >>= call "sqrtf"
                    Log   -> mapM k2llvmExp exps >>= call "logf"
                    Log10 -> mapM k2llvmExp exps >>= call "log10f"
                    Exp   -> mapM k2llvmExp exps >>= call "expf"
                    Sin   -> mapM k2llvmExp exps >>= call "sinf"
                    Cos   -> mapM k2llvmExp exps >>= call "cosf"
                    Tan   -> mapM k2llvmExp exps >>= call "tanf"
                    Asin  -> mapM k2llvmExp exps >>= call "asinf"
                    Acos  -> mapM k2llvmExp exps >>= call "acosf"
                    Atan  -> mapM k2llvmExp exps >>= call "atanf"
                    Sinh  -> mapM k2llvmExp exps >>= call "sinhf"
                    Cosh  -> mapM k2llvmExp exps >>= call "coshf"
                    -- Error
